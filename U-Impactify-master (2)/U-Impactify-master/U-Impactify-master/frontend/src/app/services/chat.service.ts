...
export class ChatService {
  ...
  postNewChat(from: string, to: string): Observable<Chat> {
    return this.http.post<Chat>(`${this.baseUrl}/api/chat`, { from, to });
  }
  ...
  getChat(chatId: string): Observable<Chat> {
    return this.http.get<Chat>(`${this.baseUrl}/api/chat/${chatId}`);
  }
  ...
  findChat(from: string, to: string): Observable<Chat> {
    return this.http.get<Chat>(`${this.baseUrl}/api/chat/find/${from}/${to}`);
  }
}
